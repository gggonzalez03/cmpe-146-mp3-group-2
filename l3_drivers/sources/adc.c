...
void adc__enable_burst_mode(adc_channel_e channel) {
  // See Table 678
  // Set corresponding channel bit in SEL in Control Register (CR)
  LPC_ADC->CR |= (1 << channel);
  // Set BURST bit in CR to 1: BURST
  LPC_ADC->CR |= (1 << 16);
  // Clear START bit in CR: No Start
  LPC_ADC->CR &= ~(UINT32_C(7) << 24);
}

uint16_t adc__get_channel_reading_with_burst_mode(adc_channel_e channel) {

  const uint16_t adc_bits = 0x0FFF;

  // See Tables 677 to 682
  // Check OVERRUNn bit in A/D Status Register (STAT) to see a conversion is lost (Optional)

  if (channel == ADC__CHANNEL_2 || channel == ADC__CHANNEL_4 || channel == ADC__CHANNEL_5) {
    // Check DONEn bit in STAT
    if (LPC_ADC->STAT & (1 << channel)) {
      // Read RESULT bits in A/D Data Registers (DR)
      return adc_bits & (LPC_ADC->DR[channel] >> 4);
    }
  }
