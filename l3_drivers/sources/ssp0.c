 ...
 *
 **********************************************************/
static void ssp0__power_on() {
  uint32_t spi_0_power_on = (1 << 21);
  LPC_SC->PCONP |= spi_0_power_on;
}

static void ssp0__setup_clock(uint32_t max_clock_mhz) {
...
static void ssp0__setup_cr0() {
  uint32_t data_size_select_8_bits = (0b111);
  uint32_t frame_format_spi = (0b00 << 4);

  LPC_SSP0->CR0 = data_size_select_8_bits | frame_format_spi;
}

static void ssp0__setup_cr1() {
  uint32_t spi_enable = 0b1 << 1;

  LPC_SSP0->CR1 |= spi_enable;
}

static void ssp0__setup_control_registers() {
  ssp0__setup_cr0();
  ssp0__setup_cr1();
}
...
void ssp0__init(uint32_t max_clock_mhz) {
  ssp0__power_on();
  ssp0__setup_clock(max_clock_mhz);
  ssp0__setup_control_registers();
}
