...
static uint16_t ascii_A[5] = {0x7E, 0x13, 0x13, 0x7F, 0x7E};

static uint8_t ascii_B_size = 6;
static uint16_t ascii_B[6] = {0x7F, 0x7F, 0x49, 0x49, 0x7F, 0x36};

static uint8_t ascii_C_size = 6;
static uint16_t ascii_C[6] = {0x1C, 0x3E, 0x63, 0x41, 0x41, 0x41};

static uint8_t ascii_D_size = 7;
static uint16_t ascii_D[7] = {0x7F, 0x7F, 0x41, 0x41, 0x63, 0x3E, 0x1C};

static uint8_t ascii_E_size = 5;
static uint16_t ascii_E[5] = {0x7F, 0x7F, 0x45, 0x45, 0x41};

static uint8_t ascii_F_size = 5;
static uint16_t ascii_F[5] = {0x7F, 0x7F, 0x05, 0x05, 0x01};

static uint8_t ascii_G_size = 7;
static uint16_t ascii_G[7] = {0x1C, 0x3E, 0x63, 0x41, 0x49, 0x79, 0x70};

static uint8_t ascii_H_size = 7;
static uint16_t ascii_H[7] = {0x7F, 0x7F, 0x08, 0x08, 0x08, 0x7F, 0x7F};

static uint8_t ascii_I_size = 2;
static uint16_t ascii_I[2] = {0x7F, 0x7F};

static uint8_t ascii_J_size = 4;
static uint16_t ascii_J[4] = {0x40, 0x40, 0x7F, 0x3F};

static uint8_t ascii_K_size = 6;
static uint16_t ascii_K[6] = {0x7F, 0x7F, 0x1C, 0x36, 0x63, 0x41};

static uint8_t ascii_L_size = 5;
static uint16_t ascii_L[5] = {0x7F, 0x7F, 0x40, 0x40, 0x40};

static uint8_t ascii_M_size = 6;
static uint16_t ascii_M[6] = {0x7F, 0x06, 0x0C, 0x06, 0x7F, 0x7F};

static uint8_t ascii_N_size = 5;
static uint16_t ascii_N[5] = {0x7F, 0x0F, 0x1C, 0x38, 0x7F};

static uint8_t ascii_O_size = 8;
static uint16_t ascii_O[8] = {0x1C, 0x3E, 0x63, 0x41, 0x41, 0x63, 0x3E, 0x1C};

static uint8_t ascii_P_size = 6;
static uint16_t ascii_P[6] = {0x7F, 0x7F, 0x09, 0x09, 0x0F, 0x06};

static uint8_t ascii_Q_size = 8;
static uint16_t ascii_Q[8] = {0x1C, 0x3E, 0x63, 0x41, 0x41, 0xE3, 0xBE, 0x9C};

static uint8_t ascii_R_size = 7;
static uint16_t ascii_R[7] = {0x7F, 0x7F, 0x09, 0x19, 0x7F, 0x66, 0x40};

static uint8_t ascii_S_size = 5;
static uint16_t ascii_S[5] = {0x46, 0x4F, 0x5C, 0x79, 0x61};

static uint8_t ascii_T_size = 6;
static uint16_t ascii_T[6] = {0x01, 0x01, 0x7F, 0x7F, 0x01, 0x01};

static uint8_t ascii_U_size = 6;
static uint16_t ascii_U[6] = {0x3F, 0x7F, 0x60, 0x40, 0x60, 0x3F};

static uint8_t ascii_V_size = 7;
static uint16_t ascii_V[7] = {0x01, 0x1F, 0x7C, 0x60, 0x78, 0x0F, 0x01};

static uint8_t ascii_W_size = 10;
static uint16_t ascii_W[10] = {0x01, 0x1F, 0x7C, 0x70, 0x1F, 0x1F, 0x70, 0x7C, 0x1F, 0x01};

static uint8_t ascii_X_size = 6;
static uint16_t ascii_X[6] = {0x63, 0x37, 0x1E, 0x3C, 0x76, 0x63};

static uint8_t ascii_Y_size = 6;
static uint16_t ascii_Y[6] = {0x07, 0x0F, 0x78, 0x78, 0x0F, 0x07};
...
void SSD1306_ascii_display_driver(uint8_t ascii_letter_size, uint16_t ascii_letter[]) {
  for (int i = 0; i < ascii_letter_size; i++) {
    SSD1306__data_write(ascii_letter[i]);
  }
}