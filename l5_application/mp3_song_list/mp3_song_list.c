...
static void mp3_song_list__handle_filename(const char *filename) {
  ...
  if (res == FR_OK) {
    for (;;) {
      ...
      if (file_info.fattrib & AM_DIR) {
        ...
        mp3_song_list__handle_filename(file_info.fname);
      }
    }
    f_closedir(&dir);
  }
...
size_t mp3_song_list__get_item_count(void) { return number_of_songs; }
